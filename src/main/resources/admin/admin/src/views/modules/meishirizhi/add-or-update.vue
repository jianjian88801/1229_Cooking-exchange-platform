<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="日志编号" prop="rizhibianhao">
            <el-input v-model="ruleForm.rizhibianhao" 
                placeholder="日志编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.rizhibianhao" label="日志编号" prop="rizhibianhao">
              <el-input v-model="ruleForm.rizhibianhao" 
                placeholder="日志编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.rizhifengmian" label="日志封面" prop="rizhifengmian">
          <file-upload
          tip="点击上传日志封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.rizhifengmian?ruleForm.rizhifengmian:''"
          @change="rizhifengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.rizhifengmian" label="日志封面" prop="rizhifengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.rizhifengmian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="美食日志" prop="meishirizhi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.meishirizhi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.meishirizhi" label="美食日志" prop="meishirizhi">
                    <span v-html="ruleForm.meishirizhi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.rizhibiaoti = true;
          ...
          if(o=='rizhifengmian'){
            this.ruleForm.rizhifengmian = obj[o];
	    this.ro.rizhifengmian = true;
            continue;
          }
          if(o=='yonghuzhanghao'){
	    ...
	    this.ro.fabushijian = true;
          ...
          if(o=='meishirizhi'){
            this.ruleForm.meishirizhi = obj[o];
	    this.ro.meishirizhi = true;
            continue;
          }
          if(o=='thumbsupnum'){
	...
	this.ruleForm.meishirizhi = this.ruleForm.meishirizhi.replace(reg,'../../../springbooted1p6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `meishirizhi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.meishirizhiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    rizhifengmianUploadChange(fileUrls) {
	this.ruleForm.rizhifengmian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
